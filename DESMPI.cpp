...
void generate_keys(string key,int rank,int size){
    int pc1[56] = {
...
string DES(){
    ...
    for(int i=0; i<16; i++) {
        ...
        string res = "";
        for(int i=0;i<8; i++){
            string row1= xored.substr(i*6,1) + xored.substr(i*6 + 5,1);//split into 8 blocks of 6 bits
            int row = convertBinaryToDecimal(row1);
            string col1 = xored.substr(i*6 + 1,1) + xored.substr(i*6 + 2,1) + xored.substr(i*6 + 3,1) + xored.substr(i*6 + 4,1);;
            int col = convertBinaryToDecimal(col1);
            int val = substition_boxes[i][row][col];
            res += convertDecimalToBinary(val);//4 bit binary string appended to res
        }
        string perm2 ="";
        for(int i = 0; i < 32; i++){
...
int main(int argc, char** argv){
    ...
    if (rank == 0) {
        generate_keys(key,rank,size); // Only master process generates keys
    }

    // Broadcast round keys to all processes
    for (int i = 0; i < 16; ++i) {
        MPI_Bcast(&round_keys[i], round_keys[i].length(), MPI_CHAR, 0, MPI_COMM_WORLD);
    }
    string ct = DES();
